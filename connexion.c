...
#include <stdlib.h>
#include <ncurses.h>
#include </home/fatoumata/Bureau/LangageC/projet/conio.h>
...
int main()
{
    ...
    strcpy(E[0].code, "1111");
    strcpy(E[0].classe.nom, "DevWeb");
    ...
    strcpy(E[1].code, "9911");
    strcpy(E[1].classe.nom, "DevWeb");
    ...
    strcpy(E[2].code, "2311");
    strcpy(E[2].classe.nom, "DevWeb");
...
sign_in:
    ...
    {
        printf("Vos données saisies étaient incorrectes. La prochaine fois, vérifiez que vous avez le bon login et le bon mot de passe.\n");
        //system("clear");
    }
    return 0;
}
...
void enregistrerEtu(ETUDIANT E[], int ne)
{
    ...
    for (int i = 0; i < ne; i++)
    {
        fprintf(file, "%d %s %s %s %s %s %d %d %d %d\n", E[i].id, E[i].nom, E[i].prenom, E[i].code, E[i].classe.nom, E[i].user.login, E[i].nbPresences, E[i].minutesretardtotal, E[i].estBloque, E[i].nbAbsences);
    }
    fclose(file);
}