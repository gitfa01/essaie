...
int convertirDate(Date date, int *jour, int *mois, int *annee);
int genererFichierPresence(Date datefin);
int convertirDate(const char *date, int *j, int *m, int *a) {
...
int main()
{
    ...
    {
        printf("Entrer une date sous format  JJ/MM/AAAA ou JJ-MM-AAAA : \n");
        sscanf(date"%d-%d-%d", &j, &m, &da);

    } while (!convertirDate(date, &j, &m, &a));
    Date dateFin = (Date){j, m, a};
